...
| `app/menu/menu_agent.py` | 질문 분석(`parse_query`), 조건/의미 검색 판단(`plan_menu_search`), 챗봇 검색 엔진 `query_menu()` |
...
업체·날짜(또는 요일)가 질문에 명확히 있으면 원본 데이터(`MenuFileStore`)를
조건 검색하고, "이번 주에 돈가스 나오는 날" 처럼 날짜 자체를 찾는 질문은 로컬
벡터 스토어로 의미 검색을 보조로 사용합니다. 의미 검색은 정확한 날짜/업체
조회의 필수 조건이 아니며, `vector_store.json`은 실행 중 생성되는 캐시
파일이라 Git에 커밋하지 않습니다.
...
- OCR은 기본적으로 Mock(sidecar 텍스트)이며, 실제 식단표 이미지에 대한 OCR 정확도는
  검증되지 않았습니다. 실제 OCR 도입 시 한글 인식 정확도, 표 구조 인식 등을 추가로 확인해야 합니다.
...
- "의미 검색이 필요한 질문"은 "언제"/"나오는" 같은 키워드로 판단하는 단순 규칙이라,
  이런 키워드 없이 특정 메뉴만 언급하는 질문은 조건 검색(오늘 날짜 기준)으로
  처리될 수 있습니다.
